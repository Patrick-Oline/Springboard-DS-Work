...
SELECT COUNT(name) AS NUM_NO_FEE 
FROM `Facilities` 
WHERE membercost = 0;
...
SELECT *
FROM (SELECT c.name , 
	 (CASE WHEN SUM(COST) < 1000 THEN SUM(COST)
      END) AS Total_Revenue
		FROM (SELECT f.facid,f.name,
			CASE 
...
SELECT CONCAT(m1.surname,', ',m1.firstname) AS member,
	CASE
	WHEN CONCAT(m2.surname,', ',m2.firstname) = 'GUEST, GUEST' THEN 'No Recommendation'
...
SELECT  b.facid, f.name, b.memid, CONCAT(m.firstname, ' ', m.surname) AS MEMBER
FROM `Bookings` b
INNER JOIN `Members` m
ON b.memid = m.memid
INNER JOIN `Facilities` f
...
SELECT MONTH(starttime) as month, f.name, COUNT(slots) AS slots, (COUNT(slots)/2) AS hours_used
FROM `Bookings` b
INNER JOIN `Facilities` f
ON b.facid = f.facid
WHERE b.memid != 0
